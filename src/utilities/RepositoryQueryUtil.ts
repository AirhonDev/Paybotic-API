 ...
 * @return {object}
 */
export const getRangeQuery = (startDate, endDate, param, table = null) => {
 ...
 * @return {object|null}
 */
export const getOrderByQuery = (orderBy, columnEquals = []) => {
	if (!orderBy) {
		return undefined
	}

	const query = orderBy.trim()
	const regex = /^\w+(?:\s(\b(asc)\b|\b(desc)\b)?)?(?:,\s\w+(?:\s(\b(asc)\b|\b(desc)\b)?)*)*$/im
	if (!regex.test(query)) {
		throw new Error(
...
export const getSearchQuery = (searchValue, columns, table?) => {
	if (searchValue) {
		const query = columns
			.map(
				(column) =>
