...
export default class SilaMoneyApiService {
	...
	constructor({ MerchantEntityRepository }) {
		this._merchantEntityRepository = MerchantEntityRepository
	}
	public async createuserEntities(
		parameters: any,
		merchantEntityPayload: any,
	): Promise<any> {
	...
	public async requestKyc(parameters: any): Promise<any> {
		...
		console.log(
			`${SILA_MONEY_API_HOST}/entities/` + parameters.handle + '/request-kyc',
		)
		const request: AxiosRequestConfig = {
			method: 'POST',
			url:
